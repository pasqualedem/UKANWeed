...
from grid import create_experiment, make_grid
...
from utils import AverageMeter, load_yaml, str2bool, write_yaml
...
def parse_args():
    ...
    parser.add_argument(
        "--loss_weights", default=False, action="store_true", help="apply loss weight"
    )
    ...
    parser.add_argument(
        "--kan_lr", default=1e-2, type=float, metavar="LR", help="initial learning rate"
    )
    parser.add_argument(
        "--kan_weight_decay", default=1e-4, type=float, help="weight decay"
    )
...
def train_epoch(config, train_loader, model, criterion, optimizer):
...
def make_name(config):
    def fmt(val):
        if isinstance(val, float):
            return f"{val:.1e}"  # scientific notation with 1 decimal
        return str(val)

    name = ""
    name += f"{config['arch']}_{config['input_list'].replace(',', '_')}"
    if config["no_kan"]:
        name += "_nokan"
    name += f"_SZ{config['input_h']}_{config['input_w']}"
    name += f"_{config['dataset']}"
    name += f"_L{config['loss']}"
    name += f"_LW{fmt(config['loss_weights'])}"
    name += f"_BS{config['batch_size']}"
    name += f"_LR{fmt(config['lr'])}"
    name += f"_KLR{fmt(config['kan_lr'])}"
    name += f"_WD{fmt(config['weight_decay'])}"
    name += f"_KWD{fmt(config['kan_weight_decay'])}"
    name += f"_SCH{config['scheduler']}"
    name += f"_E{config['epochs']}"
    name += f"_PAT{config['patience']}"
    name += f"_ES{config['early_stopping']}"
...
def main():
    ...
    if config.get("cfg") is not None:
        config = load_yaml(config["cfg"])
        
        config_grid = create_experiment(config)
    else:
        config_grid = [config]
        
    print(f"Running {len(config_grid)} runs")
    for i, run_parameters in enumerate(config_grid):
        run_parameters["name"] = make_name(run_parameters)
        print(f"Running run {i+1}/{len(config_grid)}")
        train(run_parameters)
...
def train(config):
    ...
    write_yaml(config, f"{output_dir}/{exp_name}/config.yml")
    ...
    if isinstance(config["input_list"], str):
        config["input_list"] = list(map(int, config["input_list"].split(","))) 
    ...
    for epoch in range(config["epochs"]):
        ...
        # train for one epoch
        train_log = train_epoch(config, train_loader, model, criterion, optimizer)
    ...
    torch.save(model.state_dict(), f"{output_dir}/{exp_name}/model.pth")
    print("Testing")
    test(output_dir, exp_name, device)
